...
| Variable | Default | Description |
|----------|---------|-------------|
| `HBC_ASSET_ID_LABEL_PATTERN` | `^100[0-9]{13}$` | Regex a QR payload must match, in full, to be accepted as an asset ID. Empty disables detection. |
| `HBC_ASSET_ID_SENTINEL` | `9000000000000000` | Asset ID of the sentinel item that keeps Homebox's own numbering above the labels (see below). `0` disables the check and the Settings action. |
...
The pattern is strict on purpose: product packaging is full of QR codes, and a loose pattern would file a marketing URL as an item's asset ID. The default matches the 16-digit ids the label generator prints: `100`, nine digits of print time, four of batch index, which Homebox shows as `100-0026839510004`. Both a bare ID and a Homebox `/a/{id}` URL are accepted. Hyphens are removed before matching, because Homebox ignores them.

**The sentinel item.** Homebox hands out asset IDs itself, always the next number above the highest one in the group: to every item it creates (while its `HBOX_OPTIONS_AUTO_INCREMENT_ASSET_ID` option is on, the default), to every item and location still without one at every startup (regardless of that option), and on Duplicate. It only ever counts upward, so a printed label is safe exactly when the highest asset ID already in Homebox is above it. Settings → Pre-printed labels shows whether that holds and offers to create the sentinel: an archived item named "Asset ID sentinel" carrying asset ID `9000000000000000`, above every label and below JavaScript's safe-integer limit. From then on everything Homebox numbers on its own reads `900-…`, labels read `100-…`, and no sticker still in the drawer can be handed out. The capture screen warns while the sentinel is missing. Do not delete the item. `HBC_ASSET_ID_AUTO_ASSIGN` changes none of this; it only adds one more sweep after each batch.
